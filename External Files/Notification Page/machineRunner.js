let model, webcam, predictionCallback, context, videoOnScreen;

// turn off button
document
  .getElementById("turn-off-camera")
  .addEventListener("click", async function() {
    await webcam.stop();
    videoOnScreen = document.getElementById("webcam-pose-canvas");
    videoOnScreen.style.width = "0px";
  });

// This function sets up the model trained in Teachable Machine (PoseNet tensorflow.js).
...
export async function setupModel(URL, predictionCB) {
  ...
  webcam = new window.tmPose.Webcam(400, 400, true); //width, height, flipped (when train, flip camera)
...
async function predict() {
  // Prediction #1: run the webcam through posenet (https://github.com/tensorflow/tfjs-models/tree/master/posenet)
